% Main script to perform A* path planning and Wi-Fi communication analysis (200 Robots)
...
% Define the main start positions (adjusting for 40 robots per room)
% We will create 40 positions for each room by replicating the 5 main positions multiple times
room1_main_positions = repmat([5, 5; 5, 10; 5, 15; 5, 20; 5, 25], 8, 1);
room2_main_positions = repmat([105, 95; 105, 90; 105, 85; 195, 95; 195, 90], 8, 1);
room3_main_positions = repmat([195, 105; 195, 110; 195, 115; 195, 120; 195, 125], 8, 1);
room4_main_positions = repmat([75, 195; 80, 195; 85, 195; 90, 195; 95, 195], 8, 1);

% Generate starting positions with random offsets for 200 robots (40 per room)
room1_positions = room1_main_positions + randn(40, 2) * 2;
room2_positions = room2_main_positions + randn(40, 2) * 2;
room3_positions = room3_main_positions + randn(40, 2) * 2;
room4_positions = room4_main_positions + randn(40, 2) * 2;

% Combine all robot positions (200 robots total)
...
trajectory_animation([room1_positions; room2_positions; room3_positions; room4_positions], goal_positions, paths, 'robot_animation_no_comm_200.mp4');
...
trajectory_animation([room1_positions; room2_positions; room3_positions; room4_positions], goal_positions, paths_with_comm, 'robot_animation_with_comm_200.mp4');
...
%title('Comparison of Path Lengths: With vs. Without Communication (200 Robots)');
grid on;
...
%title('Comparison of Computation Times: With vs. Without Communication (200 Robots)');
grid on;
...
%title('Comparison of Travel Times: With vs. Without Communication (200 Robots)');
grid on;
