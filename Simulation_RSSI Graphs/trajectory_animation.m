function trajectory_animation(start_poses, goal_poses, paths, video_filename)
    ...
    for i = 1:size(start_poses, 1)
        plot(start_poses(i, 1), start_poses(i, 2), 'go', 'MarkerSize', 3, 'MarkerFaceColor', 'g');  % Start positions
        plot(goal_poses(i, 1), goal_poses(i, 2), 'ro', 'MarkerSize', 3, 'MarkerFaceColor', 'r');    % Goal positions
    end
    ...
    for i = 1:num_robots
        ...
        robot_markers(i) = plot(start_poses(i, 1), start_poses(i, 2), 'bo', 'MarkerSize', 3, 'MarkerFaceColor', 'b');
    end
