% Main script to perform A* path planning and Wi-Fi communication analysis (10 Robots)
...
% Generate starting positions with random offsets for 10 robots (2 per room)
room1_positions = repmat(room1_main_positions(1:2, :), 1, 1) + randn(2, 2) * 2;
room2_positions = repmat(room2_main_positions(1:2, :), 1, 1) + randn(2, 2) * 2;
room3_positions = repmat(room3_main_positions(1:2, :), 1, 1) + randn(2, 2) * 2;
room4_positions = repmat(room4_main_positions(1:2, :), 1, 1) + randn(2, 2) * 2;

% Combine all robot positions (10 robots total)
...
trajectory_animation([room1_main_positions; room2_main_positions; room3_main_positions; room4_main_positions], goal_positions, paths, 'robot_animation_no_comm_10.mp4');
...
trajectory_animation([room1_main_positions; room2_main_positions; room3_main_positions; room4_main_positions], goal_positions, paths_with_comm, 'robot_animation_with_comm_10.mp4');

%% Old Graphs (Restored) and Saving as PDFs
...
%title('Comparison of Path Lengths: With vs. Without Communication (10 Robots)');
grid on;
...
%title('Comparison of Computation Times: With vs. Without Communication (10 Robots)');
grid on;
...
%title('Comparison of Travel Times: With vs. Without Communication (10 Robots)');
grid on;
