% Main script to perform A* path planning and Wi-Fi communication analysis (20 Robots)
...
% Generate starting positions with random offsets for 20 robots (5 per room)
room1_positions = repmat(room1_main_positions(1:5, :), 1, 1) + randn(5, 2) * 2;
room2_positions = repmat(room2_main_positions(1:5, :), 1, 1) + randn(5, 2) * 2;
room3_positions = repmat(room3_main_positions(1:5, :), 1, 1) + randn(5, 2) * 2;
room4_positions = repmat(room4_main_positions(1:5, :), 1, 1) + randn(5, 2) * 2;

% Combine all robot positions (20 robots total)
...
trajectory_animation([room1_main_positions; room2_main_positions; room3_main_positions; room4_main_positions], goal_positions, paths, 'robot_animation_no_comm_20.mp4');
...
trajectory_animation([room1_main_positions; room2_main_positions; room3_main_positions; room4_main_positions], goal_positions, paths_with_comm, 'robot_animation_with_comm_20.mp4');
...
%title('Comparison of Path Lengths: With vs. Without Communication (20 Robots)');
grid on;
...
%title('Comparison of Computation Times: With vs. Without Communication (20 Robots)');
grid on;
...
%title('Comparison of Travel Times: With vs. Without Communication (20 Robots)');
grid on;
