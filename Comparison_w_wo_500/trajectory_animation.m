function trajectory_animation(start_poses, goal_poses, paths, video_filename)
    ...
    % Maximum allowable index in start_poses
    max_index = size(start_poses, 1);

    for i = 1:num_robots
        room_id = ceil(i / 25);  % Identify the room of the robot (1 to 4)
        pos_id = mod(i-1, 5) + 1;  % Cycle through the 5 main start positions

        % Calculate the index safely, ensuring it does not exceed bounds
        index = min((room_id-1)*5 + pos_id, max_index);

        % Plot initial position of each robot based on their main start position
        robot_markers(i) = plot(start_poses(index, 1), start_poses(index, 2), 'bo', 'MarkerSize', 2, 'MarkerFaceColor', 'b');
    end
